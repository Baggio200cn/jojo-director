...
**科学实验类 AI 微课创作画布 —— 把"教学准确性"写进产线，而不是写进祈祷词。**

定位 Web 端产品，首期聚焦**光学**学科（逐步扩展到机械、电子、物理、化学、生物、计算机、自动化）。从一段文字脚本开始，自动完成 **脚本 → 分镜 → 首/尾帧 → 视频 → 质检 → 拼接成片** 的全流程，产出带字幕字卡的教学微课视频。首个完整实测作品：《LED驱动器与智慧照明控制》12 镜 / 2分08秒 / 全程无人工剪辑（成片见 [Releases](../../releases)）。
...
- **素材库 + 自我学习飞轮**（v0.4 新增）：素材越用越准，规则越审越厚——详见下文

## 素材库与"自我学习"飞轮（v0.4）

机器"学会"的不是梯度，而是**越来越厚的规则库 + 样例库 + 改判记录**。JOJO 不做模型微调，学习发生在三个确定性沉淀层：

1. **素材库（样例库）**：按学科目录组织（光学类 / 基础医学教学类…），首批种子为 PhET 仿真（CC-BY 4.0，逐条标注授权）；教师可投稿，投稿自动向量化入库
2. **Embedding 语义层**：`doubao-embedding-vision` 多模态向量（2048 维）——素材语义检索、投稿自动建议目录、改判记录向量化，全链路降级保护（无 Key 时自动退回关键词匹配）
3. **改判沉淀闭环（规则的入料口）**：验收台上每一次人工改判（fail_type + 原因）落库 `qc_corrections` 并自动向量化；`backend/tools/induce_rules.py` 定期把改判记录喂给 LLM 归纳成**草稿规则**（DRAFT，入 `qc_rules/_drafts.yaml`），经教研审核后转正为正式质检规则

> 路由规则不是一次性写死的：老师投稿越多、改判越多，归纳出的规则越贴近真实教学场景——这是 JOJO 的"自动路由"能持续变准的原因。
...
- **管理员**：账号密码登录，可在网页端直接管理邀请码（新建 / 停用 / 设每日视频条数与费用额度）
- **教师**：凭邀请码登录，顶栏实时显示自己的今日用量；邀请码是准入凭证，被邀请者无发码权限
- 生成费用由平台承担，额度按邀请码粒度限流

## 目录结构

```
backend/
  app/                 FastAPI 后端：节点执行器、Ark 网关、质检、MAAO 路由引擎、Embedding 语义层
  prompts/             极简提示词（热加载，改文件即生效；legacy/ 存放已淘汰的重型版）
  qc_rules/            领域质检规则包（光学/机械/运动学…）+ 失败分类词表 + _drafts.yaml 草稿规则
  tools/               induce_rules.py —— 把改判记录归纳成草稿质检规则（LLM，JSON 输出）
...
frontend/              React + React Flow 画布：首页 / 画布 / 验收台 / 素材库（含教师投稿）
