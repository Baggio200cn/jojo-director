...
- video {prompt, resolution: 480p|720p|1080p, duration: 3|5|10}（自动用上游图像作首帧；正式成片一律用 1080p）
- code_render {template: lens_focus|pwm_waveform|spectrum_recipe|block_diagram|rotary_drill_station|michelson_fringes|michelson_lightpath}
...
- enhance {segment_index, slow_factor, zoom_region, label_text, caption, own_material: 是|否}（R1 真实素材增强：对真实片段慢放/特写/标注，零费用零幻觉——实验演示类首选。权利硬闸：own_material≠是 的片段拼接时会被排除——第三方参考素材只学结构，禁止原样进片）
